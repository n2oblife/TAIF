    ...
    def parse(self, instruction: str) -> Optional[Dict]:
        ...
        try:
            ...
            if match:
                ...
                # Ensure 'files' is a list if present
                if 'files' in intent and isinstance(intent['files'], str):
                    intent['files'] = [f.strip() for f in intent['files'].split(',') if f.strip()]
                return intent
        except Exception:
            pass
        return None 